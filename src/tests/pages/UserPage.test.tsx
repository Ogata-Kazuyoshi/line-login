...
describe('UserPage.tsxのテスト',()=> {
    test('レンダーされたとき、正しい要素が表示されている',()=>{
        ...
        expect(screen.getByText('Lineから取得したUserIdは下記です')).toBeInTheDocument()
        expect(screen.getByText('test-user')).toBeInTheDocument()
        expect(screen.getByRole('button',{name : 'ログアウトする'})).toBeInTheDocument()
        expect(screen.getByRole('button',{name : 'IDトークンの検証をする'})).toBeInTheDocument()
    })

    test('ログアウトボタンを押すと、logoutRepositoryを正しい引数で呼び、その返り値のstatusが200の場合、useNavigate()に/を引数として呼ぶ',async ()=>{
        vi.stubGlobal("localStorage",{
            getItem : vi.fn().mockImplementation(params => params === 'access_token' ? 'test-access-token' : null)
        })
