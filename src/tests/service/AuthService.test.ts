...
describe("AuthService.tsxのテスト", () => {
  ...
  describe("checkAccessTokenServiceを呼ぶと", () => {
    test("access_tokenがnullでないかつ,checkTokenRepositoryがundefinedでなければ,getUserIdServiceの返り値を返す", async () => {
      vi.mocked(checkTokenRepository).mockImplementation((param) =>
        Promise.resolve(
      ...
      vi.mocked(getUserIdRepository).mockImplementation((param) =>
        Promise.resolve(
          param === "test-access-token" ? { sub: "test-user-id" } : undefined,
        ),
      );

      const res =
        await AuthService.checkAccessTokenService("test-access-token");
